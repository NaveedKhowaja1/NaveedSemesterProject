...
class PrintData implements ActionListener{
    ...
    String ply_id,name,father,address,phone,email,education,post,age,dob,CNIC;
    JButton b2;
    ImageIcon icon;
    ...
    PrintData(String e_id){
        try{
            Conn con = new Conn();
            String str = String.format("SELECT * FROM `player` WHERE `Player Id` LIKE '%s'",e_id);
            ...
            while(rs.next()){
                ...
                father = rs.getString("Father's Name");
