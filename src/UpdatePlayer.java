...
class UpdatePlayer extends AddPlayer implements ActionListener{
    ...
    void showData(String s){
        try{
            Conn con = new Conn();
            String str =String.format("SELECT * FROM `player` WHERE `Player Id` LIKE '%s'",s);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
                ...
                String str =String.format("UPDATE `player` SET `Name`='%s',`Father's Name`='%s',`Address`='%s'," +
                        "`Email Id`='%s',`Education`='%s',`Player Id`='%s',`CNIC`='%s'," +
                        "`Phone`='%s' WHERE `Player Id`='%s'",t1.getText(),t2.getText(),t3.getText(),t4.getText(),
                        t5.getText(),t6.getText(),t8.getText(),t9.getText(),id_ply);
