...
class RemovePlayer implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                Conn con = new Conn();
                String str = "select Name,Phone,Email Id from player where Player Id ='"+t.getText()+"' ";
                str=String.format("SELECT Name,Phone,`Email Id` FROM `player` WHERE `Player Id` LIKE '%s'",t.getText());
        ...
        if(ae.getSource()==b1){
            try{
                Conn con = new Conn();
                String str =String.format("DELETE FROM `player` WHERE `Player Id`='%s'",t.getText());
